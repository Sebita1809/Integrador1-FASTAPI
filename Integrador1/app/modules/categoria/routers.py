...
@router.get(
    "/", response_model=schemas.CategoriaRead, status_code=status.HTTP_200_OK
)
...
def detalle_categoria(id: int = Path(..., gt=0)):
    categoria = services.obtener_por_id
    if not categoria:
        raise HTTPException(
...
@router.put(
    "/{put}", response_model=schemas.CategoriaRead, status_code=status.HTTP_200_OK	
)
...
@router.put(
    "/{íd}", response_model=schemas.CategoriaRead, status_code=status.HTTP_200_OK
)
def borrado_logico(id: int = Path(..., gt=0)):
    desactivada = services.desactivar(id)
    if not desactivada:
        raise HTTPException(
            status_code=HTTP_404_NOT_FOUND, detail="Categoria no encontrada"
        )
    return desactivada